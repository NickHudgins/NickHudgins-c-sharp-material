...
FROM country 
ORDER BY surfacearea, continent DESC
...
SELECT name, (gnp-gnpold) AS [gnp Difference], (gnpold-gnp) AS [gnp Absolute]
FROM country

-- 17. The average population of cities in each country (hint: use city.countrycode)
-- ordered from highest to lowest.
-- (highest avg population: 4017733.0000, "SGP")
SELECT AVG=(name+countrycode)
FROM city

	
-- 18. The count of cities in each state in the USA, ordered by state name.
-- (45 rows)
SELECT district AS [State Name], COUNT(name) AS [Number of Cities]
FROM city
WHERE countrycode = 'USA'
GROUP BY district
ORDER BY district 
...
FROM country
ORDER BY continent, [Number of Countries] DESC
